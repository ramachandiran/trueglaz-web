...
export function saveActor(actor: Actor | null) {
  try {
    ...
    /* private mode — the app still works, it just forgets the actor */
  }
}
...
async function request<T>(path: string, actor: Actor | null, init?: RequestInit): Promise<T> {
  let res: Response
  try {
    ...
    throw new ApiError(0, 'Could not reach the API. Is it running on the proxied origin?')
  }
